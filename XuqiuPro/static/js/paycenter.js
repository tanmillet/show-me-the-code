$(document).ready(function(){
	...
	$(".bodyleftbottom .tr1").click(function(){
		...
		coininfo.moneyrate = $(this).attr('bili');
		switch(paytype){
			case "1":
				//银行卡（易宝）
				$(".platcoin2").hide();
	...
	$(document).on('click','.sl label',function(){
		...
		$.post(url,"username="+username+"&sid="+sid,function(ret){
			if(!obj.role_name){
				alert('无法找到对应角色！');
			...
			success:function(data){
				...
				if(len <= 100){
					...
					for(var i = 0;i < len;i++){
						...
						var temphtml = "<input type='radio' name='slist' value='" + obj[i]['sid'] + "'/>&nbsp;<label sid='" + obj[i]['sid'] + "' quick='" + quick + "'>" + obj[i]['name'] + "</label>";
	...
	$.countcoin = function(){
		...
		if(!coininfo.coinname || !coininfo.gamerate || !coininfo.moneyrate || !coininfo.money){
			...
			$("input[name='gamemoney']").val(coininfo.money * coininfo.gamerate * coininfo.moneyrate);
		}
	}
	...
	$(".page1submit").on('click',function(){
		...
		var poststring = $("form[name='form2']").serialize();
		...
		$.ajax({
			...
			success:function(data){
                ...
                if(obj.code == 1){
                    //成功返回，进入第二步
                    $("#step1").hide();
                    //$("#step2").html(obj.msg.confirm.forminfo);
                    $(".con-orderid").html(obj.msg.order_no);
                    $('.con-username').html(obj.msg.user.username);
                    $('.con-money').html(obj.msg.money);
                    var payname = "";
                    switch (obj.msg.confirm.method){
                        case 'alipay':
                            payname = '支付宝';
                            break;
                        case 'yeepay':
                        	payname = obj.msg.confirm.bankname;
                        	//console.log(payname);
                        	break;
                        case 'platform':
                        	payname = '平台币支付';
                    ...
                    $('.bottombtn').append(obj.msg.confirm.forminfo);
                    $("#step2").show();
                }else{
                    alert(obj.msg);
                }
    ...
    if(step3){
    	...
    	$('.three-money').html(step3info.amount);
    	$('.three-time').html(step3info.time);
    	$('.three-paymethod').html(step3info.pay);
		$("#step1").hide();
		$('#step2').hide();
		$('#step3').show();
	}
